...
impl TuiApp {
    ...
    fn refresh_core(&mut self) {
        ...
        match DesktopApplicationScanner::from_options(options).discover() {
            ...
            Err(error) => {
                self.availability =
                    vec![RegisteredApplicationAvailability::Unknown; self.registry.apps.len()];
    ...
    fn toggle_selected(&mut self) {
        ...
        let store = RegistryStore::new(&self.config_path);
        match store
            .mutate(|registry| registry.set_enabled(std::slice::from_ref(&desktop_id), desired))
        {
            Ok(_) => {
                self.refresh_core();
                self.set_ok(format!(
        ...
        match DesktopApplicationScanner::from_options(options).discover() {
            Ok(report) => {
                ...
                self.add_candidates = report
                    ...
                    .collect();
                self.add_candidates.sort_by(|left, right| {
                    left.name
                        .cmp(&right.name)
...
fn draw_header(frame: &mut Frame<'_>, app: &TuiApp, area: Rect) {
    ...
    let title = Line::from(vec![
        ...
        Span::raw(format!(
            "  E-cores {}  {startup}",
            format_cpus(&app.efficiency_cpus)
        )),
    ]);
    frame.render_widget(
...
fn draw_applications(frame: &mut Frame<'_>, app: &TuiApp, area: Rect) {
    let items: Vec<ListItem<'_>> = if app.registry.apps.is_empty() {
        vec![ListItem::new(
            "No registered applications. Press a to add one.",
        )]
    } else {
        app.registry
            .apps
...
fn draw_configure_modal(frame: &mut Frame<'_>, app: &TuiApp, area: Rect) {
    ...
    let mut lines = vec![Line::from(draft.desktop_id.clone()), Line::from("")];
